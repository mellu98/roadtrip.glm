export async function generatePDF(itinerary) {
  ...
  const opt = {
    margin: [0, 0, 0, 0],
    filename: `${itinerary.title || 'itinerary'}.pdf`.replace(/[^a-zA-Z0-9_\-àèéìòù]/g, '_'),
...
function getWeatherInfo(weatherForecasts, date) {
  ...
  const info = WMO_CODES[forecast.weatherCode] || { emoji: '🌡️', desc: 'N/A' }
  return {
    emoji: info.emoji,
    desc: info.desc,
...
function buildPDFContent(itinerary) {
  ...
  container.innerHTML = `
    ...
    ">
      ...
      <p style="
        ...
        margin-top: auto;
        padding-top: 40px;
      ">Generated by RoadTrip Planner — ${today}</p>
    </div>
  `
  ...
  days.forEach((day, i) => {
    ...
    summaryHtml += `
      <tr style="border-bottom: 1px solid ${colors.border};">
        ...
        <td style="padding: 8px 12px; font-size: 13px; color: ${colors.primary};">${day.theme || 'Day ' + (i + 1)}</td>
  ...
  summaryHtml += `
    <div style="
      ...
      font-size: 10px;
      color: ${colors.textLight};
    ">Generated by RoadTrip Planner — ${today}</div>
    </div>
  `
        ...
        ">
          <div>
            <div style="
              ...
              color: ${colors.accent};
              margin-bottom: 8px;
            ">DAY ${day.dayNumber || dayIdx + 1}</div>
            <h2 style="
              font-family: 'Playfair Display', Georgia, serif;
              font-size: 26px;
              color: ${colors.primary};
              margin: 0 0 6px 0;
            ">${day.theme || 'Day ' + (dayIdx + 1)}</h2>
        ...
        ">
          <thead>
            <tr style="border-bottom: 2px solid ${colors.accent};">
              <th style="padding: 8px 12px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600; width: 55px;">Time</th>
              ...
              <th style="padding: 8px 12px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600;">Activity</th>
              <th style="padding: 8px 12px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600; width: 60px;">Duration</th>
              <th style="padding: 8px 12px; text-align: right; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600; width: 70px;">Price</th>
              <th style="padding: 8px 12px; text-align: center; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600; width: 50px;">Rating</th>
            </tr>
          </thead>
          <tbody>
    `
    ...
    dayHtml += `
        ...
        ${day.dailyCost ? `
          <div style="
            ...
            border-top: 1px solid ${colors.border};
          ">
            <span style="font-size: 12px; color: ${colors.textSecondary};">Estimated daily cost: </span>
    ...
    let budgetHtml = `
      <div style="padding: 40px; page-break-after: always;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          <thead>
            <tr style="border-bottom: 2px solid ${colors.accent};">
              <th style="padding: 8px 12px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600;">Category</th>
              <th style="padding: 8px 12px; text-align: right; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600;">Estimated</th>
              <th style="padding: 8px 12px; text-align: right; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600;">Actual</th>
              <th style="padding: 8px 12px; text-align: right; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: ${colors.textLight}; font-weight: 600;">Difference</th>
            </tr>
          </thead>
          <tbody>
    `
